...
**All data is going to be converted in a tree xml structure. This meant there is a 'root' element and arrays are converted in items.**
...
**The csv data is also going to be converted to xml. Then you can select the desirable elements with Xpath. To select this item you can use //item[name='olaf'], or //root/_ and get all items. With this you can also use mustache templates.**
...
"mapping": {
  "id": "{{ source_1.id }}",
  "name": "{{ source_1.name | string.lower }} {{ source_1.lastName | string.lower }}",
